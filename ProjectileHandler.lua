...
local GLOBAL_TIME_SCALE = 0.9
local GLOBAL_ACCELERATION = -Vector3.new( 0, workspace.Gravity * 0.35, 0 )
...
function Module:CreateArrowProjectile( LocalPlayer, Origin, Velocity )
	...
	projectileData.OnRayHit = function( self, raycastResult )
		...
		local debugMessage = string.format(
			'Default OnRayHit: Projectile (%s) has RayHit at Vector3 (%s) with Instance (%s)',
			tostring( self.ID ),
...
RunService.Heartbeat:Connect(function( dt )
	local index = 1
	while index <= #ActiveProjectiles and task.wait() do
